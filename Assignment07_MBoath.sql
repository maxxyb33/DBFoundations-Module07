...
Begin Try
	Use Master;
	If Exists(Select Name From SysDatabases Where Name = 'Assignment07DB_MaxBoath')
	 Begin 
	  Alter Database [Assignment07DB_MaxBoath] set Single_user With Rollback Immediate;
	  Drop Database Assignment07DB_MaxBoath;
	 End
	Create Database Assignment07DB_MaxBoath;
End Try
Begin Catch
	Print Error_Number();
End Catch
go
Use Assignment07DB_MaxBoath;
...
SELECT
vP.ProductName,
DATENAME(mm, CAST(vI.InventoryDate as DATE)) + ', ' + DateName(yy, CAST(vI.InventoryDate as DATE)) as [InventoryDate] , 
[Count]
FROM vProducts as vP
INNER JOIN vInventories as vI ON
vP.ProductID = vI.ProductID
ORDER BY 1, CAST(vI.InventoryDate as Date)
...
AS
	...
	ProductName, 
	DateName(mm, CAST(InventoryDate as DATE)) + ', ' + DateName(yy, CAST(InventoryDate as DATE)) as [InventoryDate], 
...
AS
	...
	dC.CategoryName, 
	-- [DELETE] = Month(dI.InventoryDate),
	InventoryDate = DateName(mm, CAST(InventoryDate as DATE)) + ', ' + DateName(yy, CAST(InventoryDate as DATE)), 
	InventoryCountByCategory = SUM(Count)
	FROM dbo.Categories as dC
	INNER JOIN dbo.Products as dP
	ON dC.CategoryID = dP.CategoryID
	INNER JOIN dbo.Inventories as dI
	ON dP.ProductID = dI.ProductID
	GROUP BY dC.CategoryName, InventoryDate
...
-- Check that it works: Select * From vProductInventoriesWithPreviouMonthCounts;  UPDATED NAME
go
